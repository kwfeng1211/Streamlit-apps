...
# # これから↓の内容はtchatGPTでerrorを対応できるのパージョンです。
# # tryが入っています。
...
# def video_search(youtube, q="python 自動化", max_results=20):
#     try:
#         search_response = youtube.search().list(
#             q=q,
#             part="id,snippet",
...
#         error_reason = err._get_reason()
#         st.error("すみません！、今日の分析できる容量は限界になりました！")
#         return pd.DataFrame(columns=["video_id", "channel_id"])
...
# df_video = video_search(youtube, q=query, max_results=20)
# results = get_results(df_video, threshold=threshold)
